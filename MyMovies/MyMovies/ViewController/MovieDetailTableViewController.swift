...
class MovieDetailTableViewController: UITableViewController {
    ...
    @IBOutlet weak var imdbRateScore: UILabel!
    ...
    func loadMovie(withID id: String){
        serviceManager.getMovie(with: id) { [weak self] (movieObject, error) in
    ...
    func setMovieInfo(with movie: MovieModel.Movie){
        DispatchQueue.main.async {
            ...
            self.serviceManager.getImage(url: movie.poster, handler: { [weak self] (data, error) in
                if let _data = data {
                    DispatchQueue.main.async {
